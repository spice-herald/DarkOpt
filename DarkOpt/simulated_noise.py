...
def dynamical_response(detector):
    ...
    TES = detector.QET.TES # tes object
...
def simulate_noise(detector):
    ...
    SI_squid = (2.5e-12) ** 2 * np.ones(n_omega) # A^2 / Hz

    dIdPt = detector.get_dIdP()
    #print("dIdPt ", dIdPt)
    #print("|dIdPt^2| ", abs(dIdPt**2))
    ...
    Spt_xtra_Sp = Spt_Gtb * TES.fSp_xtra ** 2
    Si_extra_Sp = Si_Gtb * TES.fSp_xtra ** 2
